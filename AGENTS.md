...
10. **Three-Tier Agent Memory (Sub-Brains & Living State):** Domain agents operate as autonomous Sub-Brains within their assigned Vault directory: (1) **Subconscious** — procedural rules (`<Domain> Lessons.md`) and conversation summary injected via DPFH into the system prompt; (2) **Short-Term Recall & Living State** — active domain documents (`My Skills.md`, etc.) updated directly via knowledge distillation, backed by session memory in `SqliteSaver` scoped per `conversation_id`; (3) **Deep Recall** — discrete atomic conversation notes (`Vault/<Domain>/Archive/Conversations/YYYY-MM-DD - <Topic>.md`), completed document archives (`Vault/<Domain>/Archive/`), and decision ledgers (`Vault/<Domain>/Logs/`) accessed on-demand via search tools. The `summarize_conversation` node enforces the ~30 message window by distilling old turns into an atomic conversation archive note before checkpointer pruning. Use the shared summarizer in `src/nexus/shared_tools/summarizer.py`; do not reimplement per-agent.
...
17. **Centralized Constitution & Lightweight Domain Prompts:** All universal memory protocols, tool usage instructions, and boundary rules MUST reside centrally in `src/nexus/core/engine_constitution.py` (injected into the static prompt prefix). Individual agent prompt files (`prompts.py`) must remain ultra-lightweight, containing ONLY the agent's persona, specialized domain workflow protocols (e.g. resume tailoring, clinical triage), and dynamic DPFH placeholders. Never duplicate generic memory mechanics in individual agent prompts.
