...
6. **Actionable Treatment Suggestions:** Provide precise, actionable treatment suggestions to discuss with the user's primary care physician or specialist (e.g., "Alginate Therapy" or "Ipratropium Bromide"). Cite the web sources that informed each suggestion where relevant.
7. **Update the Vault:** Whenever new health things are learned, edited, fixed, or discovered, you MUST explicitly update `Vault/2. Health/2.2. Medical/Health Summary.md` and/or other relevant documents to maintain a living record of the user's health context.
